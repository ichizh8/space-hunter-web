...
// ---- Types -----------------------------------------------------------------
interface DragState {
  type: 'pan' | 'move_entity' | 'place_circle' | 'surface_paint' | 'minimap_pan';
  startScreenX: number;
  startScreenY: number;
  startWorldX: number;
  startWorldY: number;
...
export function EditorCanvas() {
  ...
  const draw = useCallback(() => {
    ...
    // ---- Surface tiles (drawn before grid) ----------------------------------
    ...
    for (const ent of entities) {
      ...
      switch (ent.type) {
        ...
        case 'obstacle': {
          const { w: ow, h: oh } = ent.variant
            ? getObstacleSize(ent.variant)
            : { w: ent.width ?? 64, h: ent.height ?? 64 };
          const ws = r2s(ow);
          const hs = r2s(oh);
    ...
    const pool = textPoolRef.current;
    for (let i = labelIdx; i < pool.length; i++) pool[i].visible = false;
    ...
    for (const ent of entities) {
      ...
      switch (ent.type) {
        ...
        case 'trigger_zone':     col = COL_TRIGGER;      break;
  ...
  const onPointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      // Drag-to-resize for circle placement tools
